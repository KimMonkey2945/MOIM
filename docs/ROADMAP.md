...
| ------- | ----------------------------------------- | ------------- | ------------- | ----- | ------------------- |
| Phase 0 | UI 토대 (내비 · 레이아웃 · 디자인 시스템) | mock          | A-04          | ☐ 0/6 | 대기 — **우선순위** |
| Phase 1 | 모임 + RSVP **UI**                        | mock          | E-01~E-08     | ☐ 0/5 | 대기                |
| Phase 2 | 공지 + 댓글 **UI**                        | mock          | N-01~N-05     | ☐ 0/3 | 대기                |
| Phase 3 | **데이터베이스 & Supabase 통합**          | 실데이터 전환 | A-04·E·N 전체 | ☐ 0/5 | 대기                |
| Phase 4 | 카풀 매칭 (풀스택)                        | 실데이터      | C-01~C-06     | ☐ 0/4 | 대기                |
| Phase 5 | 정산 (풀스택)                             | 실데이터      | X-01~X-06     | ☐ 0/4 | 대기                |
| Phase 6 | 주최자 대시보드 (풀스택)                  | 실데이터      | D-01~D-05     | ☐ 0/5 | 대기                |

**📊 진행 상황:** 0/32 Tasks 완료 (0%)
...
### Phase 0 — UI 토대 (내비 · 레이아웃 · 디자인 시스템) — **우선순위**
...
- [ ] **Task 000: 라우트 트리 전체 스캐폴딩 + mock 데이터 토대** `A-04` `P1`
...
- [ ] **Task 001: 디자인 시스템 & 공통 스타일 (PRD 7장)**
...
- [ ] **Task 004: 공통 레이아웃 통합 (`/protected/layout.tsx`)**
...
- [ ] **Task 005: 프로필 화면 UI (display_name · mock/로컬 상태)** `A-04` `P1`
...
- [ ] **Task 006: 이벤트 생성/수정 폼 UI** `E-01` `E-02` `P0`
...
- [ ] **Task 007: 홈 피드 — 이벤트 카드 목록 UI** `E-03` `E-07` `E-08` `P0/P1`
...
- [ ] **Task 008: 이벤트 상세 + 탭 골격 UI** `E-04` `E-06` `P0`
...
- [ ] **Task 009: RSVP 3-segment 컨트롤 UI (로컬 상태)** `E-05` `P0`
...
- [ ] **Task 010: 참여자 아바타 스택 + 참여자 목록** `E-06` `P0`
...
- `npm run lint` + `npm run build` 통과
- **Playwright E2E (mock 핵심 루프):** 이벤트 생성 폼 → 홈 피드 카드 → 상세 진입 → RSVP 로컬 토글 → 아바타 스택 → 필터/검색. 엣지: 유효성 실패, 빈 피드, 잘못된 eventId
...
- [ ] **Task 011: 공지 피드 + 공지 폼 UI** `N-01` `N-02` `N-03` `P0`
...
- [ ] **Task 012: 핀 고정 공지 UI (로컬 상태)** `N-03` `P0`
...
- [ ] **Task 013: 댓글 스레드 + 댓글 폼 UI** `N-04` `N-05` `P0`
...
- `npm run lint` + `npm run build` 통과
- **Playwright E2E (mock):** 공지 작성→핀 고정→댓글 작성→본인 댓글 삭제. 엣지: 권한별 노출 차이, 빈 입력, 핀 단일 보장
- **🎯 M1 게이트:** Phase 0~2 완료 = 클릭 가능한 UI 프로토타입(mock) 완성. UI 보완점·필드 누락을 점검해 Phase 3 스키마에 반영
...
- [ ] **Task 014: 전체 스키마 마이그레이션 (profiles·events·participants·announcements·comments)** `A-04` `E-01` `N-01` `P0`
...
- [ ] **Task 015: 전체 RLS 정책 + 헬퍼 함수 + 타입 재생성** `P0`
...
- [ ] **Task 016: 모임 + RSVP wire-up (mock → 실데이터)** `E-03` `E-04` `E-05` `P0`
...
- [ ] **Task 017: 공지 + 댓글 wire-up (mock → 실데이터)** `N-01` `N-04` `P0`
...
- [ ] **Task 018: 프로필 display_name wire-up + Phase 3 통합 테스트** `A-04` `P0`
...
- Supabase MCP `list_tables` / `get_advisors`로 전체 테이블 RLS 점검(경고 0건)
- **Playwright E2E:** Task 016~018의 실데이터 플로우 통합 통과 = **M2 게이트**
...
**📅 최종 업데이트:** 2026-06-16
**📊 진행 상황:** Phase 0 대기 중 (0/32 Tasks 완료, 0%)
